...
vec4 ComputeLines(vec4 light_col, vec4 diffuse, vec3 normal, vec3 position)
{
	float surface_int = -dot(normal, normalize(position)) ;
	if (surface_int > .25)
		surface_int = 1;
	else
		surface_int = 0;
	vec4 lambert = diffuse * surface_int;
...
{
	float surface_int = max(dot(normal, -vec3(0, 0, 1)), 0.0f);
	if (surface_int < .1)
		surface_int = 1;
	else
		surface_int = 0;
	vec4 lambert = diffuse * light_col * surface_int;
...
void main()
{
	...
	vec4 lines = ComputeLines(vec4(1, 1, 1, 1), u_Diffuse, normalize(v_Normal), pos);

	fragcolor = vec4(lines.xyz, 1);
};